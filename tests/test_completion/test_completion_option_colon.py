    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            ...
            "_CLIGENIUS_COMPLETE_ARGS": "colon_example.py --name ",
        },
    )
    assert "alpine\\\\:hello" in result.stdout
    assert "alpine\\\\:latest" in result.stdout
...
def test_completion_colon_zsh_partial():
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "_COLON_EXAMPLE.PY_COMPLETE": "complete_zsh",
            "_CLIGENIUS_COMPLETE_ARGS": "colon_example.py --name alpine",
        },
    )
    assert "alpine\\\\:hello" in result.stdout
    assert "alpine\\\\:latest" in result.stdout
...
def test_completion_colon_zsh_single():
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "_COLON_EXAMPLE.PY_COMPLETE": "complete_zsh",
            "_CLIGENIUS_COMPLETE_ARGS": "colon_example.py --name alpine:hell",
        },
    )
    assert "alpine\\\\:hello" in result.stdout
    assert "alpine\\\\:latest" not in result.stdout
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            ...
            "_CLIGENIUS_COMPLETE_ARGS": "colon_example.py --name ",
            "_CLIGENIUS_COMPLETE_WORD_TO_COMPLETE": "",
        },
    )
    assert "alpine:hello" in result.stdout
    assert "alpine:latest" in result.stdout
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "_COLON_EXAMPLE.PY_COMPLETE": "complete_powershell",
            "_CLIGENIUS_COMPLETE_ARGS": "colon_example.py --name alpine",
            "_CLIGENIUS_COMPLETE_WORD_TO_COMPLETE": "alpine",
        },
    )
    assert "alpine:hello" in result.stdout
    assert "alpine:latest" in result.stdout
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "_COLON_EXAMPLE.PY_COMPLETE": "complete_powershell",
            "_CLIGENIUS_COMPLETE_ARGS": "colon_example.py --name alpine:hell",
            "_CLIGENIUS_COMPLETE_WORD_TO_COMPLETE": "alpine:hell",
        },
    )
    assert "alpine:hello" in result.stdout
    assert "alpine:latest" not in result.stdout
