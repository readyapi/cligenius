...
%(complete_func)s() {
  eval $(env _TYPES_COMPLETE_ARGS="${words[1,$CURRENT]}" %(autocomplete_var)s=complete_zsh %(prog_name)s)
}

compdef %(complete_func)s %(prog_name)s
"""

COMPLETION_SCRIPT_FISH = 'complete --command %(prog_name)s --no-files --arguments "(env %(autocomplete_var)s=complete_fish _TYPES_COMPLETE_FISH_ACTION=get-args _TYPES_COMPLETE_ARGS=(commandline -cp) %(prog_name)s)" --condition "env %(autocomplete_var)s=complete_fish _TYPES_COMPLETE_FISH_ACTION=is-args _TYPES_COMPLETE_ARGS=(commandline -cp) %(prog_name)s"'
...
$scriptblock = {
    ...
    $Env:_TYPES_COMPLETE_ARGS = $commandAst.ToString()
    $Env:_TYPES_COMPLETE_WORD_TO_COMPLETE = $wordToComplete
    ...
    $Env:_TYPES_COMPLETE_ARGS = ""
    $Env:_TYPES_COMPLETE_WORD_TO_COMPLETE = ""
}
