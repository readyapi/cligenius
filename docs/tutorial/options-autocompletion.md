...
It required the callback function to take exactly the 3 arguments `ctx`, `args` and `incomplete` in that exact order, instead of matching them dynamically based on types, as Cligenius does.
...
However, Cligenius continues to use the `autocompletion` functionality as described on this page.
